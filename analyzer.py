...
def display_consumption(sorted_list, key_dict):
    string = ''
    for item in sorted_list:
        string += key_dict[item[0]] + ': ' + str(item[1]) + '\n'
...
def count_consumption(record_list):
    ...
    for record in record_list:
        if not count_dict.has_key(record.id):
            count_dict[record.id] = change_time_to_int(record.total_time)
            key_dict[record.id] = record.key
        else:
            count_dict[record.id] += change_time_to_int(record.total_time)

    sorted_list = sorted(count_dict.iteritems(), key=lambda d:d[1], reverse = True )  
    #display the result
    display_consumption(sorted_list, key_dict)
...
def main():
    ...
    if argv_len < 2:
        ...
        log_path = validate_path(sys.argv[1])
